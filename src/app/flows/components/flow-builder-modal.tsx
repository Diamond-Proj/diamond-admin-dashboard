...
export function FlowBuilderModal({
  ...
  const [formData, setFormData] = useState<Partial<FlowBuilderFormData>>({});
  ...
  const updateFormData = (stepData: Partial<FlowBuilderFormData>) => {
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 0: // Endpoint step
        return (
          formData.gce_endpoint &&
          formData.function_code
        );
      case 1: // Container step
        return formData.function_code;
      case 2: // Build step
        return true;
      case 3: // Review step
        return true;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: FlowImageBuilderPayload = {
        data_endpoint: formData.data_endpoint || '',
        data_path: formData.data_path || '',
        gce_endpoint: formData.gce_endpoint || '',
        function_code: formData.function_code || '',
        image: formData.image || '',
      };

      const response = await fetch('/api/flows/submit', {
        method: 'POST',
        headers: {
