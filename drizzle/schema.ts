import { pgTable, varchar, text, foreignKey, timestamp, boolean, bigint, primaryKey, json } from "drizzle-orm/pg-core"
...
export const task = pgTable("task", {
	...
	taskStatusChangedTime: timestamp("task_status_changed_time", { mode: 'string' }).defaultNow(),
}, (table) => [
	foreignKey({
...
export const dataset = pgTable("dataset", {
	...
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "dataset_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
...
export const endpoints = pgTable("endpoints", {
	...
	primaryKey({ columns: [table.endpointUuid, table.identityId], name: "endpoints_endpoint_uuid_identity_id_pk"}),
]);
